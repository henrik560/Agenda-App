...
export const Modal = ({ modalOpen, marginTop, marginLeft, saveModal, addContactPerson, closeModal, listOfUsers }) => {
    ...
    const setSelectedUserHandler = (user) => {
        ...
        var input = document.getElementsByClassName("reservation-person-placeholder");
        if(input) {
            input[0].innerHTML = user
        }
        setUserChanged(true)
        setUserListState()
    }
                ...
                >
                    ...
                    <div className="reservation-form position-relative d-flex justify-content-center align-items-center w-60 rounded-5">
                        <div className="position-relative flex-column d-flex justify-content-center align-items-center w-75">
                            ...
                            <div key="input-person" className="d-flex w-100 flex-column">
                                <div className="d-flex justify-content-between align-items-center flex-row">
                                    <div 
                                        form="reservation-form"
                                        onClick={setPersonFocusHandler}
                                        className="reservation-person-placeholder title w-100 text-truncate mt-3 border-none rounded-5 d-flex justify-content-start align-items-center " 
                                        name="resCPerson" 
                                        id="resCPerson"
                                        style={{color: '#8e8e8e'}}
                                        >
                                            ...
                                            >

                                                {
                                                  listOfUsers.users.map((user, index) => {
                                                      return ( 
                                                          <div onClick={() => setSelectedUserHandler(user.name)} className="user-item" key={user.name}>{user.name}</div>
                                                      )
                                                  })  
                                                }
