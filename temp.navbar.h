...
import{Modal} from './buildings/modal' import TableContent from './buildings/table-content';
                   ...
                   class Buildings extends React.Component
{
    ...
    componentDidMount()
    {
        this.setModalState = this.setModalState.bind(this) this.setCurrentPage = this.setCurrentPage.bind(this);
        this.fetchBuildings();
    }
    ...
    setListAmount = async(e) = > {
                               ...
                               fetchBuildings = async() = > {this.setState({refresh : true, buildingsInChunks : []}) await axios.get(`${config.baseurl} / api / buildings /`).then(response = >
                                                                                                                                                                                              {
                                                                                                                                                                                                  var buildings = [];
                                                                                                                                                                                                  Object.values(response.data).flat().map((el, id) = > buildings.push(el)) this.setState({buildingsInChunks : this.splitInChunks(buildings, this.state.listAmount), buildings})
    ...
    setCurrentPage(newPage){
        var newPage = parseInt(newPage) this.setState({currentPage : newPage})}

    filterListOnKeyWord(keyWord)
    {
        var buildingsArray = this.splitInChunks(this.state.buildings.filter((word) = > word.name.startsWith(keyWord)), this.state.listAmount) if (buildingsArray.length == 0)
        {
